...
int random() // -10 ~ 100 까지 난수 생성. 엘리베이터가 이동할 층을 무작위로 결정하는데 사용
{
    ...
    do {
        random = rand() % 111 - 10;  // -10부터 100까지의 랜덤한 수를 생성합니다.
    } while (random == 0);  // 0인 경우 다시 랜덤한 수를 생성합니다.
...
/** @brief 사용자의 버퍼를 지우는 함수.
*  @date 23/11/19
*/
void clear_input_buffer() {
    int c;// 변수 c를 선언합니다. getchar() 함수의 반환 값을 저장하는데 사용됩니다.
    while ((c = getchar()) != '\n' && c != EOF); //함수를 통해 문자를 읽어옵니다.읽어온 문자가 줄바꿈('\n')이거나 파일의 끝(EOF)을 나타내는 값이 아닌 동안 계속해서 문자를 읽어옵니다.이렇게 하면 줄바꿈 문자나 파일의 끝을 만날 때까지 입력 버퍼를 비웁니다.
}
...
/** @brief 비밀번호 유효성 함수.
*  @date 23/02/06
*  @return 숫자와 특수 문자가 모두 포함되어 있다면 유효한 것으로 판단하고 true를 반환한다.
*/
bool is_valid_password(const char* password) {
    int length = strlen(password);// 비밀번호의 길이를 구한다.
    if (length < 8)// 길이가 8 미만이면 유효하지 않다고 판단한다.
        return false;

    // 숫자와 특수 문자 포함 여부를 확인하기 위한 변수를 초기화한다.
    bool has_digit = false;
    bool has_special = false;

    // 비밀번호의 각 문자를 확인한다.
    for (int i = 0; i < length; i++) {
        // 숫자면 has_digit을 true로 변경한다.
        if (isdigit(password[i]))
            has_digit = true;
        // 알파벳이 아닌 경우 특수 문자로 판단하고 has_special을 true로 변경한다.
        else if (!isalpha(password[i]))
            has_special = true;
    }

    // 숫자와 특수 문자가 모두 포함되어 있다면 유효한 것으로 판단하고 true를 반환한다.
...
void select_elevator_for_maintenance() {
    int pick = 2; // 사용자의 선택을 받을 변수를 초기화합니다.
    int lift = 2; // 점검 설정/해제를 받을 변수를 초기화합니다.
    int count = 0; // 점검 설정된 엘리베이터의 수를 세는 변수를 초기화합니다.

    while (1) { // 무한 반복문을 시작합니다.
        printf("돌아가기는 1번을 점검을 진행은 0을 입력하세요 >> "); // 사용자에게 돌아가기 또는 점검을 진행할 것인지 선택하도록 요청합니다.
        if (scanf("%d", &pick) != 1) { // 사용자의 선택을 입력받습니다.
            printf("잘못 입력하셨습니다. 다시 입력해 주세요 (0 또는 1 입력): "); // 잘못된 입력을 받은 경우, 오류 메시지를 출력합니다.
            clear_input_buffer(); // 입력 버퍼를 지웁니다.
            continue; // 다시 선택하도록 루프의 처음으로 돌아갑니다.
        }
        if (pick == 0) { // 사용자의 선택이 점검을 진행하는 경우라면,
            while (1) { // 무한 반복문을 시작합니다.
                printf("점검 해제는 0번, 점검 설정은 1번, 돌아가기는 2번을 입력하세요 >> "); // 사용자에게 점검 해제, 설정, 돌아가기를 선택하도록 요청합니다.
                if (scanf("%d", &lift) != 1) { // 사용자의 선택을 입력받습니다.
                    printf("잘못 입력하셨습니다. 다시 입력해 주세요 (0, 1, 2 입력): "); // 잘못된 입력을 받은 경우, 오류 메시지를 출력합니다.
                    clear_input_buffer(); // 입력 버퍼를 지웁니다.
                    continue; // 다시 선택하도록 루프의 처음으로 돌아갑니다.
                }

                // 점검 설정을 선택한 경우, 엘리베이터를 두 대까지 점검 설정할 수 있다.
                if ((lift == 1) && (count == 2))
                {
                    printf("엘리베이터는 최대 2대만 설정할 수 있습니다.\n");
                    continue;

                }
                if (lift == 1) { // 점검 설정을 선택한 경우,
                    int elevator; // 점검할 엘리베이터를 선택하도록 요청합니다.
                    printf("점검할 엘리베이터를 선택해주세요 (1-6): ");
                    if (scanf("%d", &elevator) != 1) { // 사용자의 선택을 입력받습니다.
                        printf("잘못 입력하셨습니다. 다시 입력해 주세요 (1-6 사이의 숫자 입력): "); // 잘못된 입력을 받은 경우, 오류 메시지를 출력합니다.
                        clear_input_buffer(); // 입력 버퍼를 지웁니다.
                        continue; // 다시 선택하도록 루프의 처음으로 돌아갑니다.
                    }

                    // 선택한 엘리베이터가 올바른 경우, 점검 모드로 전환합니다.
                    if (elevator >= 1 && elevator <= 6) {
                        if (ele.status[elevator - 1] == 1) // 이미 점검 중인 엘리베이터인 경우, 오류 메시지를 출력합니다.
                        {
                            printf("이미 점검중인 엘리베이터 입니다.\n");
                        }
                        else // 점검 중이 아닌 엘리베이터인 경우, 점검 모드로 전환합니다.
                        {
                    ...
                    else { // 올바르지 않은 엘리베이터 번호를 입력한 경우, 오류 메시지를 출력합니다.
                        printf("잘못된 엘리베이터 번호입니다.\n");
                    }
                }
                else if (lift == 0) { // 점검 해제를 선택한 경우,
                    int elevator; // 점검을 해제할 엘리베이터를 선택하도록 요청합니다.
                    ...
                    // 선택한 엘리베이터가 올바른 경우, 점검 모드를 해제합니다.
                    if (elevator >= 1 && elevator <= 6) {
                ...
                else { // 사용자의 선택이 돌아가기인 경우, 루프를 종료합니다.
                    break;
                }
            }
        }
        else if (pick == 1) { // 사용자의 선택이 돌아가기인 경우, 함수를 종료합니다.
            break;
        }
        else { // 사용자의 선택이 잘못된 경우, 오류 메시지를 출력합니다.
...
*/
int Inspection_confirm(int i) // 엘리베이터의 점검 상태를 확인하고 처리하는 함수를 정의합니다.
{
...
void kg(int i) // 탑승자가 탑승할 때 엘리베이터의 무게를 계산하는 함수를 정의합니다.
{
    ...
    ele.user_floor[i] = NULL; // 사용자의 현재 층을 임의의 값으로 설정합니다.
...
void admin_display()  // 관리자 화면을 출력하는 함수를 정의합니다.
{
    ...
    for (int i = 0; i < MAX_ELEVATORS; i++) {  // 각 엘리베이터에 대해 반복합니다.
        if ((ele.status[i] == 3)|| (ele.status[i] == 1)) {  // 해당 엘리베이터가 점검 중인 경우
...
void admin_move_elevator()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ele.user_floor[i] = random();//6번 엘리베이터의 유저 위치를 랜덤으로 설정합니다.
        ele.target[i] = random();//1번 엘리베이터의 타겟 층 위치를 랜덤으로 설정합니다.
        ele.floor[i] = random(); // 각 엘리베이터의 초기 위치를 무작위로 설정합니다.
        ele.direction[i] = (ele.user_floor[i] > ele.floor[i]) ? 1 : -1;
    }
    ...
    while (1) {
        ...
        admin_display();//display_elevator_status 함수를 호출하여 엘리베이터의 상태를 출력.
        if (_kbhit()) {  // 키보드 입력이 있는지 체크
            if (_getch() == '\r') {  // 사용자가 엔터 키를 누르면 관리자 모드로 돌아갑니다.
               break;  // 관리자 모드 함수를 호출합니다.
            }
        }
...
void user_interface() {
    ...
    // e가 0, 1, 2 중 하나가 아니면, 올바른 입력을 받을 때까지 반복
    while (e != 0 && e != 1 && e != 2) {
...
void display_elevator_status() // 엘리베이터의 상태를 출력하는 함수를 정의합니다.
{
    ...
    for (int i = 0; i < MAX_ELEVATORS; i++) { // 모든 엘리베이터에 대해 반복합니다.
        if ((ele.status[i] == 3)|| (ele.status[i] == 1)) { // 만약 엘리베이터가 점검중인 경우,
...
void move_elevator()
{
    ...
    while (1) {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            ele.floor[i] += ele.direction[i];//엘리베이터를 이동시킵니다.
            if ((ele.floor[i] == ele.user_floor[i]) && (ele.user_floor[i] != -11))//사용자가 탑승한 엘리베이터일 경우
            {
                printf("%d번 탑승\n", i + 1);//탑승햇다는 문구 출력
                if (index == i) {   // 선택된 엘리베이터가 1번일 때
                    ...
                    ele.user_floor[i] = -11;//사용자 층을 임의의 수로 변환
                }
                kg(i);//임의의 탑승자의 무게를 증가.
            }
            if (ele.floor[index] == ele.target[index] && ele.kg[index] > 0) {
                get_off(i); // 탑승자의 무게를 제외시킵니다.
                arr = 1;
            }
            else if (ele.kg[i] > 0) {
...
int main()
{
    ...
    {   
        ...
        user_interface();//adminmode 함수 호출
